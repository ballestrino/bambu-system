...
export const OpsPageHeader = ({
  ...
  <div
    className={cn(
      "relative flex flex-col gap-4 md:flex-row md:items-end md:justify-between",
  ...
  >
    <div className="min-w-0 space-y-1">
      {eyebrow ? (
        <p className="text-xs font-semibold tracking-wide text-ops-bamboo-strong">
          {eyebrow}
        </p>
      ) : null}
      <div className="flex flex-wrap items-center gap-3">
        <h1 className="text-2xl font-semibold tracking-tight text-ops-text md:text-3xl">
          {title}
        </h1>
        {meta}
      </div>
...
export const OpsToolbar = ({ children, className, summary }: OpsToolbarProps) => (
  <div className={cn(opsSurface.toolbar, className)}>
    <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
      ...
      {summary ? (
        <div className="flex min-h-11 shrink-0 flex-wrap items-center gap-2 text-sm text-ops-text-muted">
          {summary}
        </div>
      ) : null}
    </div>
    ...
    {title || description || actions ? (
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div className="space-y-1">
          {title ? <h2 className="text-lg font-semibold text-ops-text">{title}</h2> : null}
          {description ? (
